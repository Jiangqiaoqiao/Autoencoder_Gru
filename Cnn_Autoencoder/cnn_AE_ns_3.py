...
class Cnn_AE_ns_3:
    def __init__(self, output_directory, input_shape, batchsize, verbose=False, **kwargs):
        # super(Cnn_AE_ns_3, self).__init__(**kwargs)
    ...
    def set_ModCallbacks(self):
        file_dir = os.path.join(self.output_directory, 'cnn_AE_ns_3')
        if not os.path.exists(file_dir):
            os.mkdir(file_dir)
        file_path = os.path.join(file_dir, 'best_model.hdf5')

        log_dir = ".\log\\fit\\cnn_AE_ns_3\\" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    ...
    def build_model(self):
        ...
        z = Conv2D(filters=self.conv3_filters, kernel_size=(h_z, self.z_kersize))(z)
        ...
        deconv1 = Conv2DTranspose(filters=self.conv2_filters, kernel_size=(h3, self.conv3_kersize), padding='same')(deconv1_unpool)
        ...
        deconv2 = Conv2DTranspose(filters=self.conv1_filters, kernel_size=(h2, self.conv2_kersize), padding='same')(deconv2_unpool)
        ...
        deconv3 = Conv2DTranspose(filters=self.input_shape[2], kernel_size=(h1, self.conv1_kersize), padding='valid')(deconv3_unpool)
        deconv3 = Cropping2D(cropping=((0, 0), (self.conv1_kersize//2, self.conv1_kersize//2)))(deconv3)
