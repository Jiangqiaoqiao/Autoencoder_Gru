...
class Cnn_AE_ns_4:
    def __init__(self, output_directory, input_shape, batchsize, verbose=False, **kwargs):
        # super(Cnn_AE_ns_4, self).__init__(**kwargs)
    ...
    def set_ModCallbacks(self):
        file_dir = os.path.join(self.output_directory, 'cnn_AE_ns_4')
        if not os.path.exists(file_dir):
            os.mkdir(file_dir)
        file_path = os.path.join(file_dir, 'best_model.hdf5')

        log_dir = ".\log\\fit\\cnn_AE_ns_4\\" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
