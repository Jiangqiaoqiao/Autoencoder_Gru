...
class Cnn_AE_ns_2:
    def __init__(self, output_directory, input_shape, batchsize, verbose=False, **kwargs):
        # super(Cnn_AE_ns_2, self).__init__(**kwargs)
    ...
    def set_ModCallbacks(self):
        file_dir = os.path.join(self.output_directory, 'cnn_AE_ns_2')
        if not os.path.exists(file_dir):
            os.mkdir(file_dir)
        file_path = os.path.join(file_dir, 'best_model.hdf5')

        log_dir = ".\log\\fit\\cnn_AE_ns_2\\" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    ...
    def build_model(self):
        ...
        z = Conv2D(filters=self.conv2_filters, kernel_size=(h_z, self.z_kersize))(z)
