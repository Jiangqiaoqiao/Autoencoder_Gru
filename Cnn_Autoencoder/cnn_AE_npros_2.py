...
class Cnn_AE_npros_2:
    ...
    def set_Config(self):
        ...
        self.conv1_incep1_filters = 8
        self.conv1_incep1_kersize = 1
        self.conv1_incep2_filters = 12
        self.conv1_incep2_kersize = 3
        self.conv1_incep3_filters = 12
        self.conv1_incep3_kersize = 5

        self.conv2_filters = 16
        self.conv2_incep1_filters = 4
        self.conv2_incep1_kersize = 1
        self.conv2_incep2_filters = 6
        self.conv2_incep2_kersize = 3
        self.conv2_incep3_filters = 6
    ...
    def set_ModCallbacks(self):
        file_dir = os.path.join(self.output_directory, 'cnn_AE_npros_2')
        if not os.path.exists(file_dir):
            os.mkdir(file_dir)
        file_path = os.path.join(file_dir, 'best_model.hdf5')

        log_dir = ".\log\\fit\\cnn_AE_npros_2\\" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    ...
    def build_model(self):
        ...
        # inception1
        conv1_incep1 = Conv2D(filters=self.conv1_incep1_filters, kernel_size=(h1, self.conv1_incep1_kersize))(
            input_layer)
        ...
        conv1_incep2 = Conv2D(filters=self.conv1_incep2_filters, kernel_size=(h1, self.conv1_incep2_kersize))(
            conv1_incep2)
        ...
        conv1_incep3 = Conv2D(filters=self.conv1_incep3_filters, kernel_size=(h1, self.conv1_incep3_kersize))(
            conv1_incep3)
        ...
        # inception1
        conv2_incep1 = Conv2D(filters=self.conv2_incep1_filters, kernel_size=(h2, self.conv2_incep1_kersize))(
            conv1_pool)
        ...
        conv2_incep2 = Conv2D(filters=self.conv2_incep2_filters, kernel_size=(h2, self.conv2_incep2_kersize))(
            conv2_incep2)
        ...
        conv2_incep3 = Conv2D(filters=self.conv2_incep3_filters, kernel_size=(h2, self.conv2_incep3_kersize))(
            conv2_incep3)
        ...
        deconv1_unpool = Lambda(unAbMaxPooling, arguments={'ksize': [1, 1, 2, 1]}, name='unAbPool1')(
            [encoder, conv2_argmax])
        deconv1 = Conv2DTranspose(filters=self.conv1_filters, kernel_size=(h2, self.deconv1_kersize), padding='same')(
            deconv1_unpool)
        ...
        deconv2_unpool = Lambda(unAbMaxPooling, arguments={'ksize': [1, 1, 2, 1]}, name='unAbPool2')(
            [deconv1, conv1_argmax])
        deconv2 = Conv2DTranspose(filters=self.input_shape[2], kernel_size=(h1, self.deconv2_kersize), padding='valid')(
            deconv2_unpool)
        deconv2 = Cropping2D(cropping=((0, 0), (self.deconv2_kersize // 2, self.deconv2_kersize // 2)))(deconv2)
