...
class Cnn_AE_nproc_2:
    ...
    def set_ModCallbacks(self):
        file_dir = os.path.join(self.output_directory, 'cnn_AE_nproc_2')
        if not os.path.exists(file_dir):
            os.mkdir(file_dir)
        file_path = os.path.join(file_dir, 'best_model.hdf5')

        log_dir = ".\log\\fit\\cnn_AE_nproc_2\\" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
    ...
    def build_model(self):
        ...
        deconv1 = Conv2DTranspose(filters=self.conv1_filters, kernel_size=(h2, self.deconv1_kersize), padding='same')(deconv1_unpool)
        ...
        deconv2 = Conv2DTranspose(filters=self.input_shape[2], kernel_size=(h1, self.deconv2_kersize), padding='valid')(deconv2_unpool)
