...
class Cnn_AE_npros_4:
    ...
    def set_ModCallbacks(self):
        file_dir = os.path.join(self.output_directory, 'cnn_AE_npros_4')
        if not os.path.exists(file_dir):
            os.mkdir(file_dir)
        file_path = os.path.join(file_dir, 'best_model.hdf5')

        log_dir = ".\log\\fit\\cnn_AE_npros_4\\" + datetime.datetime.now().strftime("%Y%m%d-%H%M%S")
